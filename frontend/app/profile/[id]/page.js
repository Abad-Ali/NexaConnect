...
// import { AtSign, Bookmark, BookmarkXIcon, CameraIcon, Heart, LayoutGrid, LogOutIcon, MessageCircle, PodcastIcon, Settings, User } from 'lucide-react'
...
//   //These three below are because of dialog for post
...
//   useEffect(() => {            // use this because need updated data every time user get follow or unfollow 
//     if (user && userProfile) {
...
//           md:pl-[calc(30vw+3.5rem)]  // 30vw width + 2 × 1.75rem margin
//           lg:pl-[calc(22%+3.5rem)]   // lg width + margin
//           max-w-7xl mx-auto text-white mt-10 md:mt-0">
...
//                         <Badge className="h-7 hidden md:inline pt-1">
//                           Owner
//                         </Badge>
//                       ):(
//                         <Badge className="h-7 hidden md:inline pt-1">
...
//                               <div className='w-fit h-fit text-slate-400 font-bold flex items-center gap-1'><Settings className='h-4 w-4'/>Settings</div>
//                               <Button onClick={logoutHandler} className='cursor-pointer'><LogOutIcon/>Logout</Button>
//                               <Link href='/acount/edit'><Button className='cursor-pointer'><User/>Edit Profile</Button></Link>
//                             </DialogContent>
//                           </Dialog>
//                         </div>
//                       )
...
//                   <p className='text-xs md:text-sm font-semibold'><span className='text-lg md:text-2xl font-bold'>{userProfile?.posts.length}</span> Posts</p>
//                   <Link href={`/followersorfollowing/${userProfile._id}`}><p className='text-xs md:text-sm font-semibold'><span className='text-lg md:text-2xl font-bold'>{userProfile?.followers.length}</span> Followers</p></Link>
//                   <Link href={`/followersorfollowing/${userProfile._id}`}><p className='text-xs md:text-sm font-semibold'><span className='text-lg md:text-2xl font-bold'>{userProfile?.following.length}</span> Following</p></Link>
//                 </div>
...
//                   <Badge className="w-fit text-sm" variants='secondary'><AtSign/><span className='pl-1' >{userProfile?.username}</span></Badge>  not using it ?
//                 </div> */}
//           </section>
//         </div>
...
//               isLoggedInUserProfile ? (
//                 <>
//                   <Link href={`/acount/edit`}><Button className='hover:cursor-pointer px-7 py-3'>Edit Profile</Button></Link>
//                   <Button onClick={()=>setOpen(true)} className='hover:cursor-pointer px-7 py-3'>Add New Post</Button><CreatePost open={open} setOpen={setOpen}/>
//                 </>
//               ) : (
//                 isFollowing ? (
//                   <>
//                     <Button onClick={handleFollow} className='hover:cursor-pointer px-7 py-3'>Unfollow</Button>
//                     <Link href={`/chat/${userProfile?._id}`}><Button className='hover:cursor-pointer px-7 pb-3 inline md:hidden'>Message</Button></Link>
//                     <Link href={"/chat"}><Button className='hover:cursor-pointer px-7 pb-3 hidden md:inline'>Message</Button></Link>
//                   </>
//                 ) : (
//                   <Button onClick={handleFollow} className='hover:cursor-pointer bg-blue-700 px-64 hover:bg-blue-800 py-3'>Follow</Button>
//                 )
//               ) 
//             }
//         </section>
...
//           <LayoutGrid onClick={()=> handleTabChange('posts')} className={`cursor-pointer text-gray-700 ${activeTab === 'posts' ? 'font-bold text-white' : ''}`}/>
//           {
//             isLoggedInUserProfile ? (
//               <Bookmark onClick={()=> handleTabChange('bookmarks')} className={`cursor-pointer text-gray-700 ${activeTab === 'bookmarks' ? 'font-bold text-white' : ''}`}/>
//             ) : ("")
//           }
//           {/*<Bookmark onClick={()=> handleTabChange('bookmarks')} className={`cursor-pointer text-gray-700 ${activeTab === 'bookmarks' ? 'font-bold text-white' : ''}`}/> */}
//         </div>
...
//                     <img onClick={() => openDialog(post)} src={post.image} alt='postimage' className='rounded-sm  w-full aspect-square object-cover'/>
//                     <div className='absolute inset-0
//                       flex items-center justify-center
//                       bg-black/20             // ✅ Glass effect with Tailwind shortcut
...
//               selectedPost &&(
//                 <div className="">
//                   <SinglePost postId={selectedPost._id} />
//                 </div>
//               )
//             }
//           </DialogContent>
//         </Dialog>
//         <div className={`${displayedPosts.length === 0 ? 'inline':'hidden'}`}>
//           {
...
//                   <CameraIcon className='h-7 w-7 md:h-10 md:w-10 text-gray-500'/>
//                 </div>
//                 <div className='flex flex-col items-center'>
//                   <h2 className='text-xl md:text-3xl font-bold font-sans'>No share photos</h2>
//                   <span className='text-xs md:text-sm'>When you share photos, they will appear on your profile.</span>
//                 </div>
//               </div>
//             ):(
//               <div className='flex flex-col justify-center items-center w-full pb-7'>
//                 <div className='p-2 border-3 border-gray-500 rounded-full w-fit cursor-pointer'>
//                   <BookmarkXIcon className='h-10 w-10 text-gray-500'/>
//                 </div>
//                 <div className='flex flex-col items-center'>
//                   <h2 className='text-xl md:text-3xl font-bold font-sans'>No bookmarks to show</h2>
//                   <span className='text-xs md:text-sm'>When you add bookmarks, they will appear on your profile.</span>
//                 </div>
//               </div>
//             )
//           } 
